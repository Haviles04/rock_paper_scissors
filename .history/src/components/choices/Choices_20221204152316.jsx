...
export default function Choices() {
  ...
  return (
    <div className={hardMode ? "grid-cols-5 grid-rows-3 w-screen h-screen m-5" : "grid-cols-3 grid-rows-2 w-screen h-screen m-5"}>
      {choices.map((choice) => (
        <div
          key={choice.id}
          className="m-5 w-[99px] h-[99px] rounded-full"
          style={{ boxShadow: `0px 7px ${choice.shadow}`, gridArea: `${choice.area}` }}
        >
          <button
