...
export default function Choices() {
  ...
  const setComputerChoice = () => {
    const choiceIndex = hardMode ? Math.floor(Math.random * 5)  : Math.floor(Math.random() * 3);
    return choiceIndex
  ...
  const handleChange = (choice) => {
    ...
    let compChoice = setComputerChoice();
    dispatch(compSelect(choices[setComputerChoice()]));
  }
          ...
          >
            <button
              className={`bg-gray-100 flex justify-center items-center ${
                hardMode ? "w-[95px] h-[95px]" : "w-[115px] h-[115px]"
