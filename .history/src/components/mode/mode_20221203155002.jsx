...
export default function Mode() {
  ...
  const handleChange = () => {
        if (gameMode === 'Easy') {
            dispatch(setChoices([{name: 'Rock', beats: 'Scissors'},{name: 'Paper', beats: 'Rock'},{name: 'Scissors', beats: 'Paper'}]));
            dispatch(setMode('Hard'));
        } else {
            dispatch(setChoices([...choices].concat([{name: 'Spock', beats:'Lizard'},{name:'Lizard', beats: 'Spock'}])));
