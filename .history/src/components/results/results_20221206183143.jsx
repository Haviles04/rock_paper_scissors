...
export default function Results() {
  ...
  return (
    ...
    <div className="grid grid-rows-2 grid-cols-5 justify-items-center w-screen">
      {selections.map((selection, index) =>
        selection ? (
          <div
            key={selection.id}
            className={`
w-[114px] h-[114px] text-center col-start-${index === 0 ? index + 2 : index + 3 } row-start-1
...
p-5 rounded-full shadow-btnInner mb-4 border-solid border-[9px] border-[${selection.border}]`}
    ...
    {winner ? (<div className="col-start-2 col-span-3 justify-self-center align-self-end row-start-2 text-center text-white text-4xl"><span >{winner}</span></div>) : null}
    </div>
