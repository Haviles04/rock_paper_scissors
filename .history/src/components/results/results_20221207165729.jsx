...
export default function Results() {
  ...
  return (
    <div className="grid grid-cols-5 grid-rows-2 gap-x-[25px] md:grid-cols-results grid-rows-1 justify-items-center h-[300px] min-w-[350px] md:max-w-[900px] md:h-[600px] self-center">
...
rounded-full m-5 shadow-[0px_8px_${selection.shadow}] md:shadow-[0px_20px_${selection.shadow}]`}
            >
              <div
                className={`bg-gray-100 flex justify-center items-center 
 w-[115px] h-[115px] md:w-[275px] md:h-[275px]
p-5 rounded-full shadow-btnInner mb-4 md:mb-10 border-solid border-[9px] md:border-[30px] border-[${selection.border}]`}
              >
                <img className="md:h-[95px] h-[50px]" alt={selection.name} src={selection.image} />
              </div>
              <span className="text-md text-white md:text-2xl">
                {index === 0 ? "You Picked" : "The House Picked"}
              </span>
            </div>
          ) : (
            <div key='choosing' className="col-start-4 row-start-1 m-5 text-center ">
              <div className=" opacity-75 mb-4 block bg-sky-100 rounded-full w-[115px] h-[115px] md:w-[275px] md:h-[275px]"></div>
              <span className="text-md text-white md:text-2xl">Choosing...</span>
            </div>
          )
        )}
      
      <div className="md:col-start-3 md:row-start-1 col-start-3 row-start-2 text-center mt-20 text-white h-[124px]">
        {winner && (
          <div className="">
            <div className=" mt-10 text-3xl md:text-5xl">{winner}</div>
            <button
              onClick={() => handleClick()}
              className="hover:scale-90 mt-10 w-[200px] hover:bg-white hover:text-blue-900 border-2 rounded-lg p-2 md:w-[200px]"
            >
              Play Again
            </button>
          </div>
