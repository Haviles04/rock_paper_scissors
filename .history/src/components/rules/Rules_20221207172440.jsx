...
export default function Rules() {
    ...
    <div className='absolute h-screen w-screen bg-white z-10 flex flex-col items-center justify-evenly text-center md:h-[600px] md:w-[600px] md:relative md:self-center md:rounded-xl md:flex-row' >
        ...
        <div>
        <span className=' text-3xl font-bold md:ml-10'>Rules</span></div>
        <div className='md:order-last md:w-full' >
        <img  alt='rules' src={hardMode ? hardModeRules : easyModeRules} /></div>
        <div className='md:mr-15'><button onClick={()=> dispatch(setShowRules(showRules))}><img alt='close' src={iconClose} /> </button></div> 
        </div>
    </div>
  )
}
