...
export default function Rules() {
    ...
    <div className='absolute h-screen w-screen bg-white z-10 flex flex-col items-center justify-evenly text-center md:h-[600px] md:w-[600px] md:relative md:self-center md:rounded-xl md:flex-row' >
        <div className='md:flex md:flex-row md:flex-wrap md:w-full md:h:full md:justify-between md:content-center'>
        <div>
        <span className=' text-3xl font-bold md:ml-11'>Rules</span></div>
        <div className='md:order-last md:w-full md:justify-self-center' >
        <img className='md:self-center' alt='rules' src={hardMode ? hardModeRules : easyModeRules} /></div>
        <div className='md:mr-11'><button onClick={()=> dispatch(setShowRules(showRules))}><img alt='close' src={iconClose} /> </button></div> 
        </div>
    </div>
  )
}
